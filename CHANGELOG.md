...
- **Release workflow** - `publish-gpr` job now installs dependencies from npmjs.org before switching registry to GitHub Packages for publish. Previously dependency install could hit GitHub Packages and fail with 401/404.
- **`files` field** - Removed non-existent `shared/` and `docs/` directories from package manifest.
- **`publishConfig`** - Removed hardcoded `npm.pkg.github.com` registry so `publish-npm` job can target npmjs.org as intended.

### Added

- npm publish provenance (`--provenance`) for supply-chain integrity.
